...
for article in df_filled['article'].unique():
    ...
    # Analyze sparsity of the item
    non_zero_days_train = (train_y > 0).sum()
    total_days_train = len(train_y)
    sale_prob = non_zero_days_train / total_days_train
    ...
    results.append({
        ...
        'mae_tsb': mae_tsb,
        'mae_prior': mae_prior,
        'tsb_beat_prior': mae_tsb < mae_prior
    })

res_df = pd.DataFrame(results)
lost_df = res_df[~res_df['tsb_beat_prior']]

print("Total items TSB lost or tied:", len(lost_df))
print("\nLost/Tied items breakdown by Category:")
print(lost_df['category'].value_counts())

print("\nSample of Lost/Tied Items:")
print(lost_df.head(10)[['article', 'category', 'sale_prob', 'mae_tsb', 'mae_prior']])

print("\nMean sale probability for items TSB won: {:.4f}".format(res_df[res_df['tsb_beat_prior']]['sale_prob'].mean()))
print("Mean sale probability for items TSB lost/tied: {:.4f}".format(lost_df['sale_prob'].mean()))
