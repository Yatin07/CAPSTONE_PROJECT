...
if __name__ == '__main__':
    ...
    # We test the top grid combination + the original manual combination
    prophet_params = [
        (0.01, 10.0, 'additive'), # Top grid result
        (0.05, 0.1, 'multiplicative') # Original manual baseline guess
    ]
    
    xgb_params = [
        (None, 0.1, 100), # Unrestricted default
        (7, 0.1, 100) # Deepest grid constraint
    ]
    ...
    for cps, sps, sm in prophet_params:
        ...
        for xgb_p, wapes in xgb_fold_wapes.items():
            ...
            results.append({
                'Prophet Config': f"CPS={cps}, SPS={sps}, SM={sm}",
                'XGB Max Depth': str(xgb_p[0]),
                'CV WAPE': mean_wape,
                'Split WAPEs': str([round(w, 4) for w in wapes])
            })
