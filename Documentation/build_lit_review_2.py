...
def append_section(title, papers):
    with codecs.open('c:/Users/yatin/Downloads/CAP/Documentation/LocalDemand_Final_Literature_Review.md', 'a', 'utf-8') as out:
        out.write(f"\\n## {title}\\n\\n")
        for paper in papers:
            out.write(f"### {paper['title']}\\n")
            if 'details' in paper:
                out.write(paper['details'] + "\\n")
            out.write(f"**Literature Review Paragraph:**\\n{paper['paragraph']}\\n\\n---\\n\\n")

sections = []

# SECTION 4
sec4 = {
    'title': 'Section 4 — Weather/External Factors',
    'papers': [
        {
...
for sec in sections:
    append_section(sec['title'], sec['papers'])

with codecs.open('c:/Users/yatin/Downloads/CAP/Documentation/LocalDemand_Final_Literature_Review.md', 'a', 'utf-8') as out:
    out.write("\\n## Base Paper Recommendation\\n\\n")
    out.write("**Primary Methodological Base Paper:**\\n")
    out.write("*Forecasting Restaurant Sales with Weather and Special Days using Facebook Prophet* (Güler et al.) OR *State-Level Drug Retail Sales Forecasting Using XGBoost and Facebook Prophet* (Kamboj et al., 2025). These share our core model (Prophet) combined with weather/holiday regressors.\\n\\n")
    out.write("**Primary Application Context Base Paper:**\\n")
    out.write("*Comparative Evaluation of SARIMA, FB Prophet, and LSTM for Hourly Demand Forecasting in Saudagar Kopi* (Aqila et al., 2026) strongly matches the small food business (SME) environment.\\n")
